...
const TMActionRelWB = () => {
  ...
  return (
    <Col md="12" className="tm-wbsinaction-delete">
      <Formik
        initialValues={formData}
        enableReinitialize
        validationSchema={Yup.object({})}
        onSubmit={(values, { setSubmitting }) => {
          formData.append("action", "unrelatewbtoaction");
          ...
          Axios.post("/", formData)
            .then(
              (response) =>
                response.status === 201 && navigate("/time/actions/wbs-list")
            )
            .catch((error) => {
              switch (error.response.data.messageTitle) {
                case "Project Token Check Failure.":
      ...
      >
        ...
        <Form>
          <div className="form-group">
            <h2>Delete Action-World-Building-Idea Relation</h2>
            {/*WB Id*/}
            {loadFormAttribute(
              "wb_id",
              "World-Building Idea",
